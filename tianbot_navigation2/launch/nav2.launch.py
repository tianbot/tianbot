...
def generate_launch_description():
    ...
    nav2_dir = os.path.join(get_package_share_directory('tianbot_navigation2'))
    map_share_dir = os.path.join(get_package_share_directory('tianbot_navigation2'), 'maps')
    path = Path(map_share_dir)
    map_src_dir =  Path(path.parents[4], "src/tianbot", *path.parts[-2:])
    nav2_launch_file_dir = os.path.join(get_package_share_directory('tianbot_navigation2'),'launch')
    rviz_config_file = os.path.join(get_package_share_directory('tianbot_rviz'),'rviz_cfg','nav2_namespaced_view.rviz')
