...
def generate_launch_description():
    ...
    map_directory = os.path.join(
        get_package_share_directory('tianbot_navigation2'), 'pbstreams')
    path = Path(map_directory)

    # operate path instance ，get the path needed
    map_directory = Path(path.parents[4], "src/tianbot", *path.parts[-2:])
    ...
    return LaunchDescription(
        [
            ...
            ExecuteProcess(
                cmd=[['ros2 service call ', f'/{default_namespace}/finish_trajectory ', 'cartographer_ros_msgs/srv/FinishTrajectory ', "'{trajectory_id: 0}'"]], 
                shell=True
            ),
            ...
            ExecuteProcess(
                cmd=[['ros2 service call ', f'/{default_namespace}/write_state ', 'cartographer_ros_msgs/srv/WriteState ', "'{filename: '", f'{map_save_config}', "' , include_unfinished_submaps: 'true'}'"]],
                shell=True
            ),
        ]
    )